...
def connect_wifi(ssid, password, oled):
    ...
    display_text(oled, "Connecting to ", "WiFi...")
    ...
    if wlan.isconnected():
        ...
        display_text(oled, "Wi-Fi connected")
        print("IP address:", wlan.ifconfig()[0])
    else:
        print("Failed to connect to Wi-Fi")
        display_text(oled, "Wi-Fi failed", "Check connection")
...
def main():
    ...
    if sensor.i2c_address not in i2c_hr_sensor.scan():
        ...
        display_text(oled, "Sensor not found")
        return
    elif not (sensor.check_part_id()):
        # Check that the targeted sensor is compatible
        print("I2C device ID not corresponding to MAX30102 or MAX30105.")
        display_text(oled, "Incompatible I2C device ID")
        return
    else:
        print("Sensor connected and recognized.")
        display_text(oled, "Sensor connected")

    # Load the default configuration
    print("Setting up sensor with default configuration.")
    display_text(oled, "Setting up ", "sensor")
    sensor.setup_sensor()
    ...
    display_text(oled, "Starting data", "acquisition")
    sleep(1)
    ...
    # Connect to Wi-Fi
    connect_wifi(SSID, PASSWORD, oled)
    ...
    try:
        ...
        display_text(oled, "Connecting to", "SMTP server...")
        ...
        display_text(oled, "Connected to", "SMTP server")
    
    except Exception as e:
        print(f"Error occurred: {e}")
        display_text(oled, "Error occurred", "connecting to", "SMTP server")
        return False

    # GPIO pin for the button
    button_pin = Pin(16, Pin.IN, Pin.PULL_UP)  # Using GPIO 16 for the button, with internal pull-up resistor

    abnormal_count = 0  # Counter for consecutive abnormal heart rate readings
    abnormal_readings = []  # List to store abnormal heart rate readings
    below_normal = True  # Flag to switch between below-normal and above-normal readings
    abnormal_mode = False  # Flag to indicate if we are in abnormal mode
    email_sent = False  # Flag to indicate if the email has been sent
    ...
    while True:
        ...
        if ticks_diff(ticks_ms(), ref_time) / 1000 > hr_compute_interval:
            ...
            if not button_pin.value():
                ...
                display_text(oled, "Button Pressed")
                abnormal_mode = not abnormal_mode  # Toggle abnormal mode
                email_sent = False  # Reset email sent flag when the button is pressed
                if abnormal_mode:
                    # Decide whether the abnormal readings should be below or above normal
                    below_normal = random.choice([True, False])
                else:
                    # Reset abnormal readings and counter when exiting abnormal mode
                    abnormal_readings.clear()
                    abnormal_count = 0
                    display_text(oled, "Normal Mode")

            if abnormal_mode:
                # Generate and display abnormal heart rate readings
                if below_normal:
                    heart_rate = random.randint(40, 59)
                else:
                    heart_rate = random.randint(101, 140)
                abnormal_readings.append(heart_rate)
                print("Abnormal HR:", heart_rate)
                display_text(oled, f"Abnormal HR {heart_rate:.0f}", "BPM")

                # Check if heart rate is outside the normal range
                if heart_rate < 60 or heart_rate > 100:
                    abnormal_count += 1
                    if abnormal_count == 5 and not email_sent:
                        subject = "ALERT: ABNORMAL HEARTRATE!!!"
                        body = f"PATIENT HEARTRATE READINGS: {abnormal_readings}"
                        ...
                        email_sent = True  # Set email sent flag
            else:
                ...
                if heart_rate == 0:
                    ...
                    if heart_rate < 60 or heart_rate > 100:
                        abnormal_count += 1
                        abnormal_readings.append(heart_rate)
                        print(f"Abnormal reading count: {abnormal_count}")
                        if abnormal_count >= 5 and not email_sent:
                            subject = "ALERT: ABNORMAL HEARTRATE!!!"
                            body = f"PATIENT HEARTRATE READINGS: {abnormal_readings}"
                            success = send_email_gmail(
                            ...
                            if success:
                                ...
                                display_text(oled, "Email failed", f"HR: {heart_rate:.0f} BPM")
                            email_sent = True  # Set email sent flag
                            abnormal_count = 0  # Reset the counter after sending email
                            abnormal_readings.clear()  # Clear the list of abnormal readings
                    else:
                        abnormal_count = 0  # Reset the counter if a normal reading is detected
                        abnormal_readings.clear()  # Clear the list of abnormal readings
                else:
